...
- В задании со `*` из прошлого ДЗ взят inventory.json и добавлена переменная `db_ip` для вывода внутреннего ip базы данных

- Изменен provision в **Packer** заменены bash скрипты на **Ansible** плейбуки, поправлена конфигурация в `app.json` и `db.json` .

- Пользователь вынесен в отдельную переменную и заменен с **ubuntu** на **appuser**.
